...
- ❌ **A runner of VS Code `launch` / `tasks` configs or an extension host** (#97). A `.code-workspace` file may carry `launch` (debugger configs), `tasks` (task-runner definitions), and `extensions.recommendations`, and a folder may carry `.vscode/extensions.json`. Logos deliberately **ignores** all of them: it has no debugger, no task runner, and no extension host, so honoring them would be meaningless. `CodeWorkspaceReader` reads only `folders`; `.vscode/settings.json` honors only the keys explicitly wired (currently `files.exclude`). This is a drawn boundary, not an omission — the VS Code keys that DO map onto a Logos concept are honored (`files.exclude`); these never will.
- ❌ **A substituter of VS Code path variables** (`${workspaceFolder}`, `${env:…}`, etc.) (#97). VS Code expands these only in settings *values* that hold paths and in `launch`/`tasks` — none of which Logos honors. The one honored setting, `files.exclude`, takes relative globs (never `${workspaceFolder}`), and `.code-workspace` `folders[].path` is resolved as relative-to-file or absolute (a `${workspaceFolder}` there would be circular). So there is no honored surface a variable could bite today. Deferred, not wired — revisit only if a future honored setting genuinely carries a path value.
- ❌ **A merger of `.logosconfig.yaml` into the `.code-workspace` protocol** (#97). Logos's per-root LaTeX/PDF build map (`.logosconfig.yaml`) stays **orthogonal per-root** (design D5, #96) rather than moving under a `.code-workspace` `settings` block. The two are independent concerns — a VS Code workspace file describes folders + editor settings; the build map is a Logos-specific preview concept — and coupling them would tie a Logos feature to the VS Code file format for no user benefit.
