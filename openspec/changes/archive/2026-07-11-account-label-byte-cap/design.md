...
**Round-2 correction (#62 verify):** giving `init` a BYTE cap fixed the smuggling but created a second-order defect in the uniquify sweep itself. The sweep built `"<label> (recovered)"` and checked uniqueness on that PRE-clamp candidate, then handed it to `init` — whose byte clamp truncates the disambiguating suffix off the end when the base label sits at/near the 256-byte cap (the base is one whole cluster over the budget, so the whole suffix is dropped). Two identical at-cap labels then survived uniquification byte-for-byte (the invariant the sweep exists to enforce), the bookkeeping sets tracked pre-clamp candidates so the realized duplicate went unnoticed, and — the net label diff being zero — `changed` never fired, so the repair was never persisted (silent + self-perpetuating). Fix: a shared `Account.labelWithSuffix(base:suffix:)` clamps the BASE to `maxLabelUTF8Bytes - suffix.utf8.count` FIRST (grapheme-boundary-safe), then appends the suffix, then byte-bounds the whole — returning EXACTLY the label `init` will store, so both the phase-3 sweep and `uniqueIsolatedLabel` uniqueness-check and bookkeep on the realized (post-clamp) label. Still NOT grapheme-capped, so #59's cleanup-clamp/suffix convergence tug-of-war is preserved (verified: the #59 exact-cap-collision convergence test still passes).
